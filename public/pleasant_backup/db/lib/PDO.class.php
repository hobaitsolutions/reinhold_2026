 ...
 */
require_once(__DIR__ . '/PDO.Log.class.php');
require_once(__DIR__ . '/PDO.Iterator.class.php');
...
class DB
{
	...
	private function TestInit($query, $parameters = null, $driverOptions = array())
	{
		if (!$this->connectionStatus)
		{
			$this->Connect();
	 ...
	 * @return array|int|null
	 */
	public function testQuery($query, $params = null, $fetchMode = PDO::FETCH_ASSOC)
	{
		$query        = trim($query);
		$rawStatement = preg_split("/( |\r|\n)/", $query);
		$this->TestInit($query, $params);
	}
